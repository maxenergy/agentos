...
- Interactive REPL：`agentos interactive` 提供 stdin 交互式命令循环（run/agents/skills/status/memory/schedule/context），默认无参数运行即进入 REPL；自由文本先交给配置的 `main` 主代理做上下文感知决策，主代理可直接回答或请求 registered capability；REPL 支持 named context、`digest|none|verbatim` context privacy、routing trace 与 `context trace tail [--pretty]` 误路由排查。详见 [docs/MAIN_REPL.md](docs/MAIN_REPL.md)
