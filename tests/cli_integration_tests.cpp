...
void WriteMainRouteActionContextCurlFixture(const std::filesystem::path& bin_dir,
    ...
    output
        ...
        << "\nJSON\n"
        << "  elif grep -q '\"role\":\"user\"' \"$body\" && grep -q '\"role\":\"assistant\"' \"$body\" && grep -q 'search news through main' \"$body\" && grep -q 'please provide query' \"$body\" && grep -q 'PENDING AGENTOS ROUTE ACTION' \"$body\" && grep -q 'news_search' \"$body\" && grep -q 'query' \"$body\"; then\n"
        << "    cat <<'JSON'\n"
