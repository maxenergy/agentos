...
inline void WriteClaudeCliFixture(const std::filesystem::path& bin_dir) {
    ...
    const auto body =
        ...
        "  exit /b 0\n"
        ")\n"
        "if \"%1\"==\"--print\" if \"%2\"==\"--output-format\" if \"%3\"==\"text\" (\n"
        "  echo claude cli response\n"
        "  exit /b 0\n"
        ")\n"
        "if \"%1\"==\"--print\" if \"%2\"==\"--output-format\" if \"%3\"==\"text\" if \"%4\"==\"--permission-mode\" if \"%5\"==\"acceptEdits\" (\n"
        "  echo claude cli response\n"
        "  exit /b 0\n"
        ")\n"
        "if \"%1\"==\"--print\" if \"%2\"==\"--output-format\" if \"%3\"==\"text\" if \"%4\"==\"--permission-mode\" if \"%5\"==\"bypassPermissions\" (\n"
        "  echo claude cli response\n"
        "  exit /b 0\n"
        ")\n"
    ...
    const auto body =
        ...
        "  exit 0\n"
        "fi\n"
        "if [ \"$1\" = \"--print\" ] && [ \"$2\" = \"--output-format\" ] && [ \"$3\" = \"text\" ]; then\n"
        "  printf '%s\\n' 'claude cli response'\n"
        "  exit 0\n"
        "fi\n"
        "if [ \"$1\" = \"--print\" ] && [ \"$2\" = \"--output-format\" ] && [ \"$3\" = \"text\" ] && [ \"$4\" = \"--permission-mode\" ] && [ \"$5\" = \"acceptEdits\" ]; then\n"
        "  printf '%s\\n' 'claude cli response'\n"
        "  exit 0\n"
        "fi\n"
        "if [ \"$1\" = \"--print\" ] && [ \"$2\" = \"--output-format\" ] && [ \"$3\" = \"text\" ] && [ \"$4\" = \"--permission-mode\" ] && [ \"$5\" = \"bypassPermissions\" ]; then\n"
        "  printf '%s\\n' 'claude cli response'\n"
        "  exit 0\n"
        "fi\n"
