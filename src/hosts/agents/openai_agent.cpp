...
bool OpenAiAgent::healthy() const {
    const auto session = credential_broker_.get_session(AuthProviderId::openai, profile_name());
...
std::string OpenAiAgent::profile_name() const {
    return profile_store_.default_profile(AuthProviderId::openai).value_or("default");
}
