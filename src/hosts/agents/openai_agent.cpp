...
constexpr char kOpenAiChatCompletionsUrl[] = "https://api.openai.com/v1/chat/completions";
constexpr char kDefaultOpenAiModel[] = "gpt-4o";
...
AgentProfile OpenAiAgent::profile() const {
    return {
        .agent_name = "openai",
        .version = "0.1.0",
        .description = "Adapter for OpenAI Chat Completions API (GPT-4o, o3, etc.) through the authenticated model provider layer.",
        .capabilities = {
            {"analysis", 85},
            {"planning", 80},
            {"code_reasoning", 85},
        ...
        // Phase 4.5: V2 path streams OpenAI SSE.
...
bool OpenAiAgent::healthy() const {
    const auto session = credential_broker_.get_session(AuthProviderId::openai, profile_name(std::nullopt));
    return session.has_value() && (CommandExists("curl") || CommandExists("curl.exe"));
}

std::string OpenAiAgent::start_session(const std::string& session_config_json) {
    (void)session_config_json;
    const auto next_id = session_counter_.fetch_add(1) + 1;
    return "openai-session-" + std::to_string(next_id);
}

std::optional<std::string> OpenAiAgent::open_session(const StringMap& config) {
    (void)config;
    const auto next_id = session_counter_.fetch_add(1) + 1;
    return std::string("openai-session-") + std::to_string(next_id);
}

void OpenAiAgent::close_session(const std::string& session_id) {
    (void)session_id;
}

AgentResult OpenAiAgent::run_task(const AgentTask& task) {
    const auto started_at = std::chrono::steady_clock::now();
    ...
    const auto session = credential_broker_.get_session(AuthProviderId::openai, profile);
    if (!session.has_value()) {
        return {
            .success = false,
            .duration_ms = ElapsedMs(started_at),
            .error_code = "AuthUnavailable",
            .error_message = "openai auth session is unavailable for profile " + profile,
        };
    }

    std::string token;
    try {
        token = credential_broker_.get_access_token(AuthProviderId::openai, profile);
    ...
    // short-lived files so the OpenAI API key never appears in argv (where it
    ...
    try {
        ...
        return {
            ...
            .duration_ms = ElapsedMs(started_at),
            .error_code = "AgentUnavailable",
            .error_message = std::string("could not stage openai request: ") + error.what(),
        };
    }
    ...
    legacy_output_json["agent"] = "openai";
    legacy_output_json["provider"] = "openai";
    ...
    AgentResult agent_result{
        ...
        .summary = result.success ? summary : "OpenAI chat completions request failed.",
...
AgentResult OpenAiAgent::run_task_in_session(const std::string& session_id, const AgentTask& task) {
...
bool OpenAiAgent::cancel(const std::string& task_id) {
    (void)task_id;
    return false;
}

// ---------------------------------------------------------------------------
// V2 invoke path: streams OpenAI SSE through curl spawned
// directly. Falls back to the legacy non-streaming path on stream failure or
// when no `on_event` callback is supplied.
// ---------------------------------------------------------------------------
AgentResult OpenAiAgent::invoke(const AgentInvocation& invocation,
    ...
    if (cancelled()) {
        return AgentResult{
            ...
            .error_message = "openai invoke cancelled before request",
        };
    }
    ...
    if (stream_outcome.cancelled || cancelled()) {
        ...
        ev.fields["error_message"] = "openai invoke cancelled";
        EmitEvent(on_event, std::move(ev));
        return AgentResult{
            .success = false,
            .duration_ms = ElapsedMs(started_at),
            .error_code = kCancelledErrorCode,
            .error_message = "openai invoke cancelled",
        };
    }
    ...
    if (stream_failed) {
        ...
        ev.payload_text = "openai streaming failed; falling back to sync";
        EmitEvent(on_event, std::move(ev));
        return fallback_to_sync();
    }
...
std::string OpenAiAgent::profile_name(const std::optional<std::string>& requested_profile) const {
    return requested_profile.value_or(profile_store_.default_profile(AuthProviderId::openai).value_or("default"));
}

std::string OpenAiAgent::model_name(const AgentTask& task) {
...
std::string OpenAiAgent::model_name_v2(const AgentInvocation& invocation) {
...
std::string OpenAiAgent::BuildPrompt(const AgentTask& task) {
...
std::string OpenAiAgent::BuildRequestBody(const AgentTask& task) {
...
std::string OpenAiAgent::BuildRequestBodyV2(const AgentInvocation& invocation, bool stream) {
    nlohmann::ordered_json message;
    message["role"] = "user";
...
std::string OpenAiAgent::ExtractFirstMessageContent(const std::string& response_json) {
    try {
...
AgentTask OpenAiAgent::InvocationToTask(const AgentInvocation& invocation) {
