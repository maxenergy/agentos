...
void PrintUsage() {
    std::cerr
        ...
        << "  agentos main-agent set provider=<openai-chat|anthropic-messages|gemini-generatecontent>\n"
        << "                         base_url=<url>\n"
        << "                         model=<model_id>\n"
        << "                         [api_key_env=ENV_VAR]\n"
        << "                         [oauth_file=<path-to-json>]\n"
...
int RunMainAgentCommand(const MainAgentStore& store, int argc, char* argv[]) {
    ...
    if (sub == "set") {
        ...
        if (config.base_url.empty()) missing.push_back("base_url");
