...
- [x] Add per-plugin process pool policy via `PluginSpec.pool_size` (capped by global `max_persistent_sessions`) with LRU eviction scoped to the same plugin name, plus TSV/JSON manifest parsing and regression coverage.
- [x] Add session admin CLI: `agentos plugins sessions`, `agentos plugins session-restart name=<plugin>`, `agentos plugins session-close name=<plugin>`, exposing the existing close/restart paths used by crash/idle eviction; surface `pool_size` in `plugins lifecycle` / `plugins inspect` output and add CLI integration coverage.
