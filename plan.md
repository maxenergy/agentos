...
| Scheduler | Partial MVP | persisted one-shot/interval tasks, `run-due`, foreground `tick` loop, run history metadata, retry/backoff | Daemon/service wrapper, cron, missed-run policy |
...
- Scheduler supports manual `run-due`, foreground `tick`, run history metadata, and retry/backoff; there is no daemon/service wrapper, cron parser, or missed-run semantics.
