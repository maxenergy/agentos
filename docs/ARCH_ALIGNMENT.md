...
- Scheduler: 🚧 支持一次性任务、interval / recurrence 任务、retry/backoff、手动 run-due、前台 tick loop 与 runtime/scheduler/runs.tsv 执行元数据，并已覆盖 disabled / missed interval 回归测试
...
* disabled 任务会跳过执行；missed interval 当前语义为每次 tick 只执行一次，并从当前 scheduler time 计算下一次运行
* 仍缺少 daemon/service wrapper、完整 cron 表达式和可配置 missed-run catch-up policy

#### 必须补充

```text
Scheduler: 🚧 已实现 add/list/remove/run-due/tick/history/retry/recurrence，并覆盖 disabled/missed interval 回归测试
...
6. Scheduler 🚧 MVP 已实现，一次性 / interval / recurrence / retry 任务可持久化，并支持 run-due / tick 执行与 run history
