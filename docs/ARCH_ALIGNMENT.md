...
- Scheduler: 🚧 支持一次性任务、interval 任务、retry/backoff、手动 run-due、前台 tick loop 与 runtime/scheduler/runs.tsv 执行元数据
...
```text
Scheduler: 🚧 已实现 add/list/remove/run-due/tick/history/retry
...
6. Scheduler 🚧 MVP 已实现，一次性 / interval / retry 任务可持久化，并支持 run-due / tick 执行与 run history
