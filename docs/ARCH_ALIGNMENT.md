...
Scheduler
- ScheduledTask: ✅ 已实现 runtime/scheduler/tasks.tsv 持久化（含 cron_expression / timezone_name 列）
- Scheduler: ✅ 支持一次性任务、interval / recurrence 任务、5-field cron 任务、retry/backoff、missed_run_policy、手动 run-due、前台 tick / daemon loop 与 runtime/scheduler/runs.tsv 执行元数据；已覆盖 disabled / missed interval / cron+timezone+DST 回归测试
- CronSupport: ✅ 5-field cron + @hourly/@daily/@weekly/@monthly/@yearly 别名 + DOM/DOW OR 语义
- TimezoneSupport: ✅ UTC + 固定偏移 (UTC±HH:MM) + 精选 IANA 命名 zone (US/EU/AU 内置 DST + 亚洲固定偏移)；spring-forward gap 跳过、fall-back fold 单次触发
...
* recurrence=every:<n>s|m|h|d 已作为轻量复现语法
* 已加入完整 5-field cron + @hourly/@daily/@weekly/@monthly/@yearly 别名，DOM/DOW 走经典 OR 语义
* ScheduledTask 增加 timezone 字段 (默认 UTC)。支持 UTC/固定偏移 (UTC+08:00) 与精选 IANA 命名 zone (US/EU/AU 内置 DST 规则；亚洲固定偏移 zone)
* DST 处理：spring-forward gap 被跳过到 gap 之后的第一个有效分钟；fall-back fold 仅在最早 (前 DST) 实例触发一次
* disabled 任务会跳过执行；missed interval 支持 run-once 或 skip stale run，并从当前 scheduler time 计算下一次运行
* TSV 兼容旧行 (没有 cron / timezone 列时按 UTC 默认加载)

#### 必须补充

```text
Scheduler: ✅ 已实现 add/list/remove/run-due/tick/daemon/history/retry/recurrence/cron/timezone/missed-run-policy，并覆盖 cron+timezone+DST 回归测试
ScheduledTask: ✅ 已实现 runtime/scheduler/tasks.tsv (含 cron_expression / timezone_name 列)
CronSupport: ✅ 5-field + @aliases；时区精选 IANA zone + 固定偏移；DST gap/fold 已处理
```

#### 已知限制 / 后续工作

* 完整 IANA tzdb (含历史规则、所有 zone) 暂未引入。当前精选表覆盖最常见的 US/EU/AU/亚洲 zone；其它 zone 需要使用 UTC+HH:MM 固定偏移或扩展 timezone.cpp 的命名表
* 未来若引入 Howard Hinnant date/tz 库或 ICU，应通过 ADR 评审并迁移到 std::chrono::tzdb 标准接口
...
6. Scheduler ✅ 一次性 / interval / recurrence / cron+timezone (含 DST gap/fold) / retry 任务可持久化，并支持 run-due / tick / daemon 执行与 run history
