...
- Scheduler: 🚧 支持一次性任务、interval / recurrence 任务、retry/backoff、手动 run-due、前台 tick / daemon loop 与 runtime/scheduler/runs.tsv 执行元数据，并已覆盖 disabled / missed interval 回归测试
...
* 仍缺少完整 cron 表达式和可配置 missed-run catch-up policy

#### 必须补充

```text
Scheduler: 🚧 已实现 add/list/remove/run-due/tick/daemon/history/retry/recurrence，并覆盖 disabled/missed interval 回归测试
